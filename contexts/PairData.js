  ...
  try {
    ...
    for (let i = 0; i < values.length - 1; i++) {
      formattedHistoryRate0.push({
        time: new Date(parseFloat(values[i].timestamp*1000)),
        open: parseFloat(values[i].rate0),
        close: parseFloat(values[i + 1].rate0),
        low:parseFloat(values[i].rate0),
        high:parseFloat(values[i + 1].rate0),
        volume:Math.abs(values[i+1].volumeToken0-values[i].volumeToken0)
      })
      formattedHistoryRate1.push({
        ...
        volume:Math.abs(values[i+1].volumeToken1-values[i].volumeToken1)
      })
    }
